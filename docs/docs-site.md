...
does not publish a public docs mirror. Production publishing happens through
the normal `homelab-docs` service deploy, which builds `.site/` on the Mac mini
runtime path and serves it through the private LAN route
`https://docs.home.feocco.com`.
