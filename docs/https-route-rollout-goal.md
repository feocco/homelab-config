...
`One eligible homelab service is converted to a LAN HTTPS route under home.feocco.com verified by generated Caddy config, UniFi DNS check/apply, strict config validation, a canary deploy, DNS resolving to the host bind address, HTTPS returning a non-error response, and strict live validation while preserving existing service containers, ports, secrets, health checks, monitoring, and rollback through the current Git branch and prior runtime config. Use only services/*/ops.yaml route fields, generated Caddy config, scripts/sync-unifi-dns, scripts/validate-service-rollout, scripts/homelab-deploy --canary, and existing docs/tests. Between iterations, choose the next eligible service from ./scripts/list-https-route-candidates --status eligible, preferring simple single-container app/proxy services with existing health checks before multi-container or externally exposed services. If blocked or no valid paths remain, report the service, blocker, evidence command/output, and the user action or repo change that would unlock progress.`
