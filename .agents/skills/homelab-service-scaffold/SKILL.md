...
```bash
python3 <path-to-this-skill>/scripts/scaffold_homelab_service.py \
  --name my-service \
  --description "Short service description" \
  --output-dir /Users/feocco/code/my-service
```

The generator lives in this skill's own `scripts/` directory; resolve the path
relative to wherever this SKILL.md was loaded from (canonical copy:
`homelab-config/.agents/skills/homelab-service-scaffold/scripts/`).
